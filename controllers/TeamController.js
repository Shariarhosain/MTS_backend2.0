...
exports.deleteTeam = async (req, res) => {
  ...
  try {
    ...
    const updatedTeam = await prisma.team.update({
      ...
      data: { 
        is_active: false ,
        deactivated_at: new Date(), // Optional: record when the team was deactivated
    },
    });

    return res.status(200).json({ message: 'Team marked as inactive (soft deleted)', team: updatedTeam });
  } catch (error) {
...
exports.teamWisechart = async (req, res) => {
    try {
        ...
        if (isSalesRole) {
            ...
            const teamTarget = teamMembers.reduce((sum, member) => sum + Number(member.target || 0), 0);
        ...
        if (isOperation) {
            ...
            for (const teamId of teamIds) {
                const teamData = await prisma.team.findFirst({
                    where: { id: teamId }, // Corrected line
                    select: {
                        team_target: true,
                        team_name: true,
                        team_member: {
                ...
                let teamTarget = teamData.team_member.reduce((sum, member) => sum + Number(member.target || 0), 0);
                ...
                res.status(200).json({
                    message: 'Team data retrieved successfully', teamTarget,
                    teamAchievement,
                    teamCancelled,
                    teamTotalCarry,
                    submitted,
