...
//     //first search department_name in department table  by  department_id
//     //from body i will get team_name, department_id, team_target. first create team
//     // then if leader_id available  set role to operation_leader if department_name not sales,also connect him in team_member table and role operation_leader
//     // if department_name sales then set role to sales_leader  for that team_member
//     // then if selectedMembers as array are available, connect them in team_member table with role operation_member if department_name not sales
//     // if department_name sales then set role to sales_member for that team_member 

//     const { team_name, department_id, team_target, leader_id } = req.body;
//     try {
        ...
        if (membersToUpdate.size > 0) {
            const teamMemberUpdates = Array.from(membersToUpdate).map(memberId => {
                ...
                if (memberId === leader_id) {
                    role = departmentName === 'sales' ? 'sales_leader' : 'operation_leader';
                } else {
                    // Assume other selected members are regular members
                    role = departmentName === 'sales' ? 'sales_member' : 'operation_member';
                }
            ...
            // Execute all team member updates in parallel
